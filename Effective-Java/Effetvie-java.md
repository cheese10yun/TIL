...
- [13. 클래스와 멤버의 접근 권한은 최소화하라](#13-클래스와-멤버의-접근-권한은-최소화하라)
    - [정보 은닉](#정보-은닉)
    - [접근 지시자](#접근-지시자)
    - [테스트 떄문에 인터페이스 또는 멤버들의 접근 권한을 열어주고 싶을 때도 있다](#테스트-떄문에-인터페이스-또는-멤버들의-접근-권한을-열어주고-싶을-때도-있다)
...
- [52. 객체를 참조할 떄는 그 인터페이스르 사용해라](#52-객체를-참조할-떄는-그-인터페이스르-사용해라)
...
- [48. 정확한 답이 필요하다면 float와 dobule은 피하라](#48-정확한-답이-필요하다면-float와-dobule은-피하라)
    - [돈을 계산할 떄는 BigDeimal, int 또는 Long을 사용해야 한단는 원칙을 지켜라](#돈을-계산할-떄는-bigdeimal-int-또는-long을-사용해야-한단는-원칙을-지켜라)
...
* 정적 멤버 클래스의 흔항 영례 가운데 하나는 바깥 클래스스와 함께 사용될떄 용의하다
...
* 떄로는 여러분은 두 가지 이상의 기능을 가지고 있으며, 그중 어떤 기능을 제공하는지 표시하는 태그(플레그)가 달린 클래스를 마날 때가 있다. 
...
```
둘 중에 어떤 방법을 써도 Long 객체에 컨테이너에 String 객체를 넣을 수는 없다. 그러나 배열을 쓰면 실수를 저지른 사실을 프로그램 실행 중에 알수 있다. **반면 리스트를 사용하면 컴파일할 떄 알 수 있다. 물론 컴파일 시에 문제를 반경하는 편이 무조건 좋다**
...
# 52. 객체를 참조할 떄는 그 인터페이스르 사용해라

* 인자의 자료형으로 클래스 대신 인터페이스를 사용해야한다.
* 객체를 참조할 떄는 클래스보다 인터페이스를 사용해야 한다는 것으로 이해할 수있다.
...
* 성능이낮다. 리플렉션을 통한 메서드 호출 성능은, 일반 메서드 호출에 비해 훨씬 낮다. 얼마나 낮은 지 정확히 마랗기는 어렵다. 고려해야 할 요건 들이 다양하기 떄문이다. 저자의 컴퓨터에서는 속도차이는 2배에서 50배 가량 차이었다.
* **명심할 것은, 일반적인 프로그램은 프로그램 실행 중에 이플랙션을 통해 객체를 이용하려 하면 안된다는 것이다.**
* **리플렉션ㅇ을 아주 제한적으로 사용하면 오버헤드는 피하면서도 리플렉션의 다양한 장점을 누릴수 있다.**
* **호출해야 하는 생성자가 아무런 인자도 받지 않을 떄는 리플랙션을 이용할 필요 조차 없다. Class.newInstance 메서드를 호출하는 것으로 충분하다.**
