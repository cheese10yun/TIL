...
# 목차
<!-- TOC -->autoauto- [목차](#markdown-header-목차)auto- [1장 도메인 모델 시작](#markdown-header-1장-도메인-모델-시작)auto    - [도메인 모델 패턴](#markdown-header-도메인-모델-패턴)auto- [2장 아키텍처 개요](#markdown-header-2장-아키텍처-개요)auto    - [네 개의 영역](#markdown-header-네-개의-영역)auto    - [계층 구조 아키텍처](#markdown-header-계층-구조-아키텍처)auto    - [DIP](#markdown-header-dip)auto        - [DIP 주의사항](#markdown-header-dip-주의사항)auto        - [DIP와 아키텍처](#markdown-header-dip와-아키텍처)auto    - [도메인 영역의 주요 구성요소](#markdown-header-도메인-영역의-주요-구성요소)auto        - [엔티티와 벨류](#markdown-header-엔티티와-벨류)auto        - [애그리거트](#markdown-header-애그리거트)auto        - [리포지터리](#markdown-header-리포지터리)auto- [3장 애그리거트](#markdown-header-3장-애그리거트)auto    - [애그리거트 루트](#markdown-header-애그리거트-루트)auto    - [도메인 규칙과 일관성](#markdown-header-도메인-규칙과-일관성)auto    - [트랜잭션 범위](#markdown-header-트랜잭션-범위)auto    - [ID를 이용한 애그리거트 참조](#markdown-header-id를-이용한-애그리거트-참조)auto        - [편한 탐색 오용](#markdown-header-편한-탐색-오용)auto        - [성능에 대한 고민](#markdown-header-성능에-대한-고민)auto        - [확장 어려움](#markdown-header-확장-어려움)auto- [4장 리포지터리와 모델구현(JPA 중심)](#markdown-header-4장-리포지터리와-모델구현jpa-중심)auto    - [매핑 구현](#markdown-header-매핑-구현)auto        - [엔티티와 벨류 기본 매핑 구현](#markdown-header-엔티티와-벨류-기본-매핑-구현)auto    - [필드 접근 방식 사용](#markdown-header-필드-접근-방식-사용)auto    - [기본 생성자](#markdown-header-기본-생성자)auto    - [별도 테이블에 저장하는 벨류 매핑](#markdown-header-별도-테이블에-저장하는-벨류-매핑)auto- [6장 응용 서비스와 표현 영역](#markdown-header-6장-응용-서비스와-표현-영역)auto    - [응용 서비스의 역할](#markdown-header-응용-서비스의-역할)auto    - [도메인 로직 넣지 않기](#markdown-header-도메인-로직-넣지-않기)auto    - [응용 서비스 구현](#markdown-header-응용-서비스-구현)auto        - [응용 서비스의 크기](#markdown-header-응용-서비스의-크기)auto        - [응용 서비스의 인테페이스와 클래스](#markdown-header-응용-서비스의-인테페이스와-클래스)auto        - [메서드 파라미터와 값 리턴](#markdown-header-메서드-파라미터와-값-리턴)auto        - [표현 영역에 의존하지 않기](#markdown-header-표현-영역에-의존하지-않기)auto        - [도메인 이벤트 처리](#markdown-header-도메인-이벤트-처리)auto    - [표현 영역](#markdown-header-표현-영역)auto        - [값 검증](#markdown-header-값-검증)auto- [7장 도메인 서비스](#markdown-header-7장-도메인-서비스)auto    - [여러 애그리거트가 필요한 기능](#markdown-header-여러-애그리거트가-필요한-기능)auto    - [도메인 서비스](#markdown-header-도메인-서비스)auto- [9장 도메인 모델과 BOUNDED COUNTEXT](#markdown-header-9장-도메인-모델과-bounded-countext)auto    - [BOUNDED CONTEXT](#markdown-header-bounded-context)auto    - [BOUNDED CONTEXT의 구현](#markdown-header-bounded-context의-구현)auto- [10장 이벤트](#markdown-header-10장-이벤트)auto    - [시스템 간 강력합의 문제](#markdown-header-시스템-간-강력합의-문제)auto    - [이벤트 개요](#markdown-header-이벤트-개요)auto    - [이벤트 관련 구성 요소](#markdown-header-이벤트-관련-구성-요소)auto        - [이벤트의 구성](#markdown-header-이벤트의-구성)auto        - [이벤트 용도](#markdown-header-이벤트-용도)auto        - [이벤트 장점](#markdown-header-이벤트-장점)auto    - [이벤트, 핸들러, 디스패처 구현](#markdown-header-이벤트-핸들러-디스패처-구현)auto        - [이벤트 클래스](#markdown-header-이벤트-클래스)auto    - [메시징 시스템을 용한 비동기 구현](#markdown-header-메시징-시스템을-용한-비동기-구현)auto- [11장 CQRS](#markdown-header-11장-cqrs)auto    - [단일 모델의 단점](#markdown-header-단일-모델의-단점)auto    - [CQRS](#markdown-header-cqrs)autoauto<!-- /TOC -->
...
**응용 서비스의 주요 역할은 도메인 객체를 사용해서 사용자의 요청을 처리하는 것임으로 표현 영역 이방에서 보았을 때 응용 서비스는 도메인 영역과 표현영역을 연결해주는 창구인 파사드 역할을 한다.**
...
두 번째 문제는 여러 응용 서비스에 동일한 도메인 로직을 구현할 가능성이 높아 진다는 것이다. 
...
그런데 응용 서비스는 보통 런타임에 이를 교체하는 경우가 거의 없을 뿐만 아니라 한 서비스의 구현 클래스가 두 개인 경우도 매우 듬물다.
...
응용 서비스에 데이터로 전달할 요청 파라미터가 두 개 이상 존재하면 데이터 전달을 위한 별도 크래스를 사용하는 것이 편리하다.

응용 서비스에서 애그리거트 자체를 리턴하면 코딩은 편할 수 있지만 도메인의 로직 실행을 응용 서비스와 표현 영역 두 곳에서 할 수 있게 된다. 이는 기능 실행 로직을 응용 서비스와 표현 영역에 분삭시켜 코드의 응집도를 낮추는 원인이 된다.

### 표현 영역에 의존하지 않기
응용 서비스의 파라미터 타입을 결정할 떄 주의할 점은 표현 영역과 관련된 타입을 사용하면 안된다는 점이다. 예를 들어 HttpServletRequest나 HttpSession을 으용 서비스에 파라미터로 전달하면 안된다. 으용 서비스에서 표현 영역에 대한 의존성이 발생하면 응용 서비스만 단독으로 테스트하기 어려워진다.

### 도메인 이벤트 처리
으용 서비스으 역할 중 하나는 더메인 영역에서 발생시킨 이벤트를 처리하는 것이다. 여기서 이벤트 도메인에서 발생한 상태 변경을 의미하면 암호 변경됨, 주문 취소함 같은 이벤트가 될 수있다.

도메인 영역은 상태가 변경되면 이를 외부에 알리기 위해 이벤트를 발생시킬수 있다. 예를 들어 암호 초기화 기능은 다음과 같이 암호 변경에 후에 암호 변경됨 이벤트를 발생시킬 수 있다.  도메인에서 이벤트를 발생시키면 그 이벤트를 받아서 처리할 코드가 필요한데 그 역할읗 하는 것이 응용 서비스이다. 응용 서비스는 이벤트를 바당서 이벤트에 알맞은 후처리를할 수있다.

## 표현 영역
표현 여역의 책음은 크게 다음과 같다
* 사용자가 시슽메을 사용할 수있는 흐름을 제공하고 제어한다.
* 사용자의 요청을 알맞은 응용 서비스에 전달하고 결과를 사용자에게 제공한다
* 사영자의 세션을 관리한다.

### 값 검증 
값 검증은 표현 영역과 응용 서비승 두 곳에서 모두 수행할 수있다. 원친적으로는 모든 값에 대한 검증은 응용 서비스에서 처리한다.

으용 서비스를 사용하는 표현 여역 코드가 한 곳이면 구현의 편리함을 위해 다음과 같이 역할을 나누어서 검증을 수행 할 수도 있다.
...
## 여러 애그리거트가 필요한 기능
도메인 영역의 코드를 작성하다 보면 한 애그리거트로 기능을 구현할 수 없을 때가 있다. 애표적인 예가 경제 금액 계산로직이다. 실제 결제 금액을 계산할 떄는 다음 과 같은 내용이 필요하다.

* 상품 애그리거트: 구매 상품의 가격이 필요하다. 또한 상품에 따라 배송비가 추가되기도 한다.
* 주문 애그리거트: 상품별로 구매 개수가 필요하다.
* 할인 쿠폰 애그리거트: 크폰 별로 지정한 할인 금액이나 비율에 따라 할인 된다.
* 회원 애그리거트: 회원 등릅에 따라 추가 할인이 가능하다.
...
처음 도메인 모델을 만들 떄 빠지기 쉬운 함정이 도메인을 완벽하게 표현하는 단일모델을만드는 시도를 하는 것이다. 그렌더 1장에서 말한 것처럼 한 도메인은 다시 여러 하위 도메인으로 구분되지 때문에 한 개의 모델로 여러 하위 도메인을 모두 표현하려고 시도하게 되면 모든 하위 도메인에 맞지 않은 모델을 만들게되된다.

하위 도메인 마다 같은 용어라도 의미가 다르고 같은 대상이라도 지칭하는 용어가 다를 수 있기 때문에 한 개의 모델로 모든 하위 도메인을 표현하는 시도는 올바른 방법이 아니며 표현할 수도 없다.

## BOUNDED CONTEXT
BOUNDED CONTEXT는 모델의 경계를 결졍하며 한 개의 BOUNDED CONTEXT는 논리저긍로 한 개의 모델을 갖는다.
...
첫 번째 문제는 이부 서비스가 정상이 아닐 경우 트랜잭션을 어떻게 처리할지가 애매하다는 것이다. 외부의 환불 서비스를 실행하는 과정에서 익셉션이 발생하면 환부렝 실패했음으로 주문 취소 트랜잭션을 롤백하는 것이 맞는 것을 보인다. 하지만 반드시 트랜잭션을 롤백해야 하는 것은 아니다. 주문은 취소 상태로 변경하고 환불만 누중에 다시 시도하는 방식으로 처리할 수도 있다.
...
이러한 강한 결갑을 업앨 수 있는 방법이 이쓴ㄴ데 그것은 바로 이벤트를 사용하는 것이다. 특히 비동기 이벤트를 사용하면 두 시스템 간의 결합을 큭레 낮출 수있다. 

## 이벤트 개요
이 절에서 사용하는 이벤트라는 용어는 '과거에 벌어진 어떤 것'을 뜻한다. 예를 들어 사용자가 비밀번호를 변경한 것은 '암호를 변경했음 이벤트' 라고 부를 수 있다.

## 이벤트 관련 구성 요소
도메인 모델에서 이벤트 주체는 엔티티, 벨류, 도메인 서비스와 같은 도메인 객체이다. 이를들 더메인 객체는 도메인 로직을 실행해서 상태가 바뀌면 관련 이벤트를 발생한다.
...
### 이벤트 용도
이벤트는 크게 두 가지 용도로 쓰인다. 첫 번째 용도는 트리거다. 도메인의 상태가 바뀔 때 다른 후처리를 해야 할 경우 후처리를 실행하기 위한 트리거로 이벤트를 사용할 수있다.

이벤트의 두 번째 용도는 서로 다른 시스템 간의 데이터 동기화이다. 배송지를 변경 하면 외부 배송 서비스에 바뀐 배송지를 정보를 전송해야한다ㅣ 이 경우 주문 도메인은 배송지 변경을 이벤트를 발생시키고 이벤트 핸들러는 외부 배송 서비스와 배송지 정보를 동기화한다.

### 이벤트 장점

이벤트를 사용하면 서로 다른 도메인 로직이 섞이는 것을 방지 할 수있다. 구매 취소 로직에 이벤트를 적용함으로써 환불 로직이 없어진다. 이벤트를 사용해서 주문 도미인에서 결제 도메인으로 의존을 제거했다.

이벤트 핸들러를 사용하면 기능 확장도 용이하다. 구매 취소 시 환불과 함께 이메일로 취소 냉용르 보내고 싶다면 이메일 발송 처리하는 핸들러 구현하고 디스패처에 등록하면 된다.
...
이벤트 구성에서 설명한 것처럼 이벤트 클래스는 이벤트를 처리하는 데 필요한 최소한의 데이터를 포함해야 한다. 예를 들어, 주문 취소 이벤트는 적어도 주문번호를 포햄해야 관련 핸들러의 후속처리를할 수있다.
...
## 단일 모델의 단점
주문 내역 조회 기능을 구현하려면 여러 애그리거트에서 데이터를 가져와야 하낟. ORder에서 주문 정보를 가져와야 하고, Product에서 상품 이름을 가져와야 하고, Member에서 회원 이름과 아이디를 가져와야 한다.
...
애그리거트 간의 연관을 ID가 아니라 직접 참조하는 방식으로 연결해도 고민거리가 생긴다. 조회 화면의 특성에 따라 연관된 즉시 로딩이나 지연로딩으로 처리해야 하기 때문이다. 떄에 따라서는 JPA의 네이티브 쿼리를 사용해야 할 수도 있다.

**이러한 고민이 발생하는 이유는 시스템의 상태를 변경할 때와 조회할 떄 단일 도메인 모델을 사용하기 때문이다.** 객체 지향 도메인 모델을 구현할 때는 주로 사용하는 ORM 기법은 도메인 상태 변경을 구현하는데 적합하지만, 주문 상 세 조회 화면처럼 여러 애그리거트에서 데이터를 가좌 추력하는 기능을 구현하기에 고려할 것들이 많아서 구현을 복잡하게 만드는 원이이 된다.

## CQRS
시스템이 제공하는 기능을 크게 두 가지로 나누어서 하나는 상태를 변경하는 기능, 사용자 입장에서 상태 정보를 조회하는 기능이다. 
