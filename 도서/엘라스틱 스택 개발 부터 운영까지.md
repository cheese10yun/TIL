...
`filter-example.log` 파일의 처음부터 다시 읽을 때 null으로 지정하면 `sinced_path` 파일을 만들지 않기 때문에 이전 파일을 읽었던 기록이 없어서 매번 로그스태시를 실행할 때 마다 파일을 처음부터 읽게 된다.
...
시각화 기능      설명
디스커버        데이터를 도큐먼트 단위로 탐색해 구조와 관계 등을 확인할 수 있다.
...
## 데이터 노드
데이터 노드는 인덱싱과 도큐먼트를 샤드 형태로 저장하여 데이터의 CRUD 작업과 검색, 집계 작업을 한다. 실질적인 데이터 프로세싱 작업이 일어나기 때문에 일반적으로 노드 중에 가장 많은 부하를 받는다. 실질적인 데이터 프로세싱 작업이 일어나기 때문에 일반적으로 노드 중애 가장 많은 부하를 받는다.
