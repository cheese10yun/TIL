...
## 다대다: 새로운 기본 키 사용
**추천하는 기본 키 생성 전략은 데이터베이스에서 자동으로 생성해주는 대리 키를 Long 값으로 사용하는 것이다** 이것의 장점은 간편하고 겅의 영구히 쓸 수 있으며 비지느스에 의존하지 않는다. 그리고 ORM 매핑시에 복합 키를 만들지 않아도 된다.
...
3. @Transcational을 선언한 메소드가 정상 종료되면 트랜잭션을 커밋하는데, 이때 영속성 컨텍스트를 종료한다. 영속성 컨텍스트가 자라졌음으로 엔티티는 이제부터 준영속 상태가 된다.
