...
**전체 100만 건의 레코드 가운데 50만 건을 익어야 하는 작업은 인덱스 손익 분기점인 20~25%보다 훨씬 크기 때문에 MySQL 옵티마이저는 인덱스를 이용하지 않고 직접 테이블을 처음 부터 끝까지 읽어서 처리할 것이다.**
...
B-Tree 인덱스에서 루트와 브랜치 노드를 이용해 특정 스캔 시작 값을 가지고 있는 리프노드를 검색하고, 그 시점부터 필요한 방향(오름 차순, 내림차순)으로 인덱스를 읽어 나가는 과정을 위 그림에서 확인할 수있다. **가장 중요한 것은 어떤 방식으로 스캔하든 관계 없이, 해당 인덱스를 수정하는 칼럼의 정순 또는 역순으로 정렬된 사앹로 레코드를 가져 온다느 것이다. 이는 별도의 정렬 과정이 수반되는 것이 아니라 인덱스 자체의 정렬 특성 때문에 자동으로 그렇게 되는 것이다.**

위 이미지에서 중요한 것은 **인덱스의 리프 노드에서 검색 조건에 칠하는 건들을 데이터 파일에서 레코드를 일거오는 과정이 필요하다는 것이다.이 때 리프 노드에 저장된 레코드 주소로 데이터 파일으 레코드를 읽어 오는데, 레코드 한 건 한 건 단위로 랜덤 I/O가 한 번씩 실행된다.** 위 그림처럼 3건이 일치하는 경우 최대 3건의 랜덤 I/O가 필요한 것이다. **그래서 인덱스를 통해 데이터 레코드를 읽는작업은 비용이 많이 드는 작업으로 뷴류되는 것이며 20~25%를 넘으면 인덱스를 통한 읽기 보다 테이블 테이터를 직접 읽는 것이 효율적인 처리 방식이 되는 것이다.**
...
**인덱스 키값은 항상 오름차순으로 정렬되지만 사실 그 인덱스를 거꾸로 끝에서부터 일긍면 내침차순으로 정렬된 인덱스로도 사용될 수 있다. 인덱스를 어느 방향으로 읽을지는 쿼리에 따라서 옵티마이저가 실시간으로 만들어내는 실행 계획에 따라 결정된다.**
...
**케이스 A는 "dept_no = 'd002' and emp_no >= 1014"인 레코드를 찾고, 그 이후에는 dept_no가 d002가 아닐 때까지 인덱스를 그냥 쭉 읽기만 하면 된다.** 이 경우에는 읽은 레코드가 모두 사용자가 원하는 결과임을 알 수 있다. 즉 5건의 레코드를 찾는 데 필요한 5번의 비교 작업만 수행 가능한 것이므로 상당히 효유ㅜㄹ적으로 인덱스를 이용한 것이다.
...
하나의 값을 검색하는경우, 유니크 인덱스와 일반 보조 인덱스는 사용되는 실행 계획이 다르다. 하지만 이는 인덱스의 성격이 유니크한지 아닌지에 따른 차이일 뿐 그 차이는 없다. 1개의 렠드를 읽느냐 2개 이상의 레코드를 읽느냐의 차이만 있다면 그것을 의미 할뿐, 읽어야 할 레코드 건수가 같다면 성능의 차이는 미미하다.
...
### 유니크 인덱스 사용 시 주의사항
반드시 필요한 겨웅에는 인덱스를 생성하는 것은 당연하지만 더 성능이 좋아질 것으로 생각하고 불필요하게 유니크 인덱스를 생성하는 것은 좋지 않다. 하나의 테이블에서 같은 칼럼에 유니크 인덱스와 일반 인덱스를 각각 중복해서 생성해 둔 경우가 가끔 있는데 MySQL의 유니크 인덱스는 일반 다른 인덱스와 같은 역할을 하므로 중복해서 인덱스를 생성할 필요는 없다. 
...
1. 사용자로부터 요청된 SQL 문장을 잘게 쪼개서 MySQL 서버가이해할 수 있는 수준으로 분리한다.
2. SQL의 파싱 정보(파스 트리)를 확인하면서 어떤 테이블로 부터 일곡 어떤 인덱스를 이용해 테이블을 읽을지 선택한다.
3. 두 번째 단계에서 결정된 테이블의 읽기 순서나 선택된 인덱스를 이용해 스토리지 엔진으로 부터 데이터를 가져 온다.

첫 번쨰 단계를 SQL 파싱 이라고하며, MySQL 서버의 SQL 파서라는 모듈로 처리 한다. 만약 SQL 문장이 문법적으로 잘못됐다면 이 단계에서 걸러진다. 또한 이 단계에서 SQL 파스 트리가 만들어진다. MySQL 서버는 SQL 문장 그 자체가 아니라 SQL 파스 트리를 이용해 쿼리를 실행한다.

두 번쨰 단계는 첫 번쨰 단계에서 만들어진 SQL 파서 트리를 참조하면서, 다음과 같은 내영을 처리한다.
...
이 밖에도 수많은 처리를 하지만, 대표적인 작업들을 정리했다. 두 번째 단계는 최적화 및 실행 계획 수립 단계이며 MySQL 서버의 옵티마이저에서 처리한다. 또한 두 번쨰 단계가 완료되면 쿼리의 실행 계획이 만들어진다.

세 번쨰 단계는 수립된 실행 계획대로 스토리지 엔진에 레코드를 일거 오도록 요청하고 MySQL 엔진에서는 스토리지 엔진으로부터 받은 레코드를 조인하거나 정렬하는 작업을 수행한다.


### 옵티마이저의 종류

옵티마이저는 현재 대부분의 DBMS가 선태갛고 있는 비용 기반 최적화 방법과 예전 오라클에서 많이 사용됐던 규칙 기반 최적화 방법으로 크게 나눠 볼 수 있다.

**규칙 기반 최적화**

규칙 기반 최적화는 기본적으로 대상 테이블의 레코드 건수나 선택도 등을 고려하지 않고 톱티마이저에 내장된 우선순위에 따라 실행 계쇡을 수립하는 방식을 의미한다. **이 방식에서는 통계 정보(테이블의 레코드 건수나 칼럼 값의 분포도)를 조사하지 않고 실행 계획이 수립되기 떄문에 같은 쿼리에 대해서는 거의 항상 같은 실행 방법을 만들어 낸다.** 이 규칙 기반의 최적화는 이미 오래 전부터 많은 DBMS에서 거의 지원되지 않거나 업데이트 되지 않은 상태로 그대로 남아있다.
...
MySQL 또한 다른 DBMS와 같이 비용 기반 최적화를 사용하지만 다른 DBMS 보다 통계 정보는 그리 다양하지 않다. 기본적으로 MySQL에서 관리 되는 통계 정보는 대략의 레코드 건수와 인덱스의 유니크한 값의 개수 정도가 전부다. 오라클과 같은 DBMS에서는 통계 정보가 상당히 정적이고 수집에 많은 시간이 요소되기 떄문에 통계 정보만 따로 백업하기도 한다. **하지만 MySQL에서 통계 정보는 사용 자가 알아채지 못하는 순간순간 자동으로 변경되기 때문에 상당히 동적인 편이다.**

ANALYZA를 사용해서 테이블의 통계 정보를 갱신하는 경우 InnoDB 테이블은 읽기와 쓰기 모두 불가능하므로 서비스 도중에 ANALYZA을 실행하지 않는 것이 좋다.

## 실행계획 분석

MySQL에서 쿼리 실행 꼐획을 확인하려면 EXPLAIN 명령을 사용하면 된다. 이번 장에서는 해당 내용에 대해서 다루겠다. 
...
| 1    | SIMPLE       | e     | NULL       | eq\_ref | PRIMARY        | PRIMARY | 8        | batch\_study.s.emp\_no | 4      | 100      | NULL        |

EXPLAIN을 실행하면 쿼리 문장의 특성에 따라 위 표형태로 1이상의 결고과가 표시된다. 표의 각 라인은 쿼리 문장에 사용된 테이블(서브 쿼리로 임시 테이블을 생성한 경우 그 임시 테이블까지 포함)의 개수만큼 출력된다. 출력된 실행 계회겡서 위쪽에 출력된 결과일수록(ID 값이 작을 수록) 쿼리의 바깥 부분이거나 먼저 접근한 부분이고, 아래쪽에 출력된 결과일수록(ID 값이 클 수록) 쿼리 안쪽 부분 또는 나중에 접그한 테이블에 해당된다.
...
### type 칼럼
**쿼리의 실행 계획에서 type 이후의 칼럼은 MySQL 서버가 각 테이블의 레코ㅓ드를 어떤 방식으로 읽었는지를 의마한다. 여기서 방식이라 함은 인덱스를 사용해 레코드를 읽었는지 아니면 테이블을 첨부터 끝까지 읽는 풀 테이블 스캔으로 레코드를 읽었는지를 의미 한다.** 일반적으로 쿼리를 튜닝할 때 인덱스를 효율적으로 사용하는지 확인하는 것이 중요하므로 실행 계획에서 type 칼럼은 반드시 체크해야할 중요한 정보다.
...
실행 계쇡의 type 칼럼은 버전 마다 명칭의 차이가 좀 있을 수 있지만 5.1 기준으로 위 처럼 표시된다. 위 12 접근 방식중에서 하단의 ALL을 제외한 나머지는 모두 인덱스를 사용하는 접근 방벙이다. ALL은 인덱스를 사용하지 않고, 테이블을 처음부터 끝까지 읽어서 레코드를 가져오는 풀 테이블 스캔 저근 방식을 의미한다. 위 type의 순서는 높을 수록 빠르다.
...
쿼리의 모양에 따라 조금은 다르겠지만 접근 방법은 ALL or indx로 표시될 가능성이 크다. **system은 테이블에 레코드가 1건 이하인 겨웅에만 사용할 수 잇는 접근 방법이므로 실제 애플리케이션에서는 사용되는 쿼리의 실행 계획에서는 거의 없다.**

#### const

**테이블의 레코드 건수에 관계없이 쿼리가 프라이머리 키나 유니크 키 말럼을 이용하는 WHERE 조건절을 가지고 있으며, 반드시 1건을 반환하는 쿼리의 처리 방식을 const라고 한다.** 다른 DBMS에서는 이를 유니크 인덱스 스캔 이라고 표현한다.
...
```
위의 쿼리에서 WHERE 절에 사용된 서브 쿼리는 employees(e2) 테이블의 프라이머리 키를 검색해서 first_name을 읽고 있다. 이 쿼리의 실행 계쇡은 다음과 같은데 예상대로 e2 테이블은 프라이머리 키를 const 타입으로 한다는 것을 볼 수 있다.
...
여기서 설명하는 것은 이 쿼리는 옵티마이저에 의해 최적화도니느 시점에 다음과 같은 쿼리로 변환된다는 것이다. **즉 옵티마이저에 의해 상수화된 다음 쿼리 실행기로 전달되기 때문에 접근 방식이 const인 것이다.**
...
**eq_ref 접근 방법은 여러 테이블이 조인되는 쿼리의 실행 계획에서만 표시된다. 조인에서 처음 읽은 테이블의 칼럼 값을, 그다음 읽어야할 테이블의 프라이머리 키나 유니크 키 칼럼의 점색 조건에 사용할 떄 req_ref라고 한다.** 이때 두 번째 이후에 읽는 테이블의 type 칼럼에 eq_ref가 표시된다, 또한 두 번째 이후에 읽히는 테이블을 유니크 키로 검색할 때 유니크 인덱스는 NOT NULL 이어야하며, 다중 칼럼으로 만들어진 프라이머리 키나 유니크 인덱스라면 인덱스의 모든 칼럼이 비교 조건에 사용되오야만 req_ref 접근 방법이 사용될 수 있다. **즉 조인에서 두 번째 이후 읽는 테이블에서 반드시 1건만 존재한다는 보장이 있어야 사용할 수 있는 접근 방법이다.**
...
**첫 번째 라인과 두번째 라인의 id가 1로 같으므로 두 개의 테이블이 조인으로 실행된다. 그리고 detp_emp 테이블이 실행 계획의 위쪽에 있으므로 detp_emp를 먼저 읽고 `e.emp_no = de.emp_no` 조건을 이용해 employees 테이블을 검색하고 있다. employees 테이블의 emp_no는 프라이머리 키라서 실행 계쇡의 두 번째 라인의 type 칼럼이 eq_ref로 표시된 것이다.**

#### ref

**ref 접근 방법은 eq_ref와는 달리 조인의 순서와 관계ㅇ벗이 사용되며, 또한 프라이머리 키나 유니크 키등의 제약 조건도 없다. 인덱스의 저ㅗㅇ류와 관계없이 동등 조건으로 검색할 때는 ref 접근 방법이 사용된다. ref는 반드시 반환되는 레코드가 1건이라는 보장이 없으므로 const, reqf 보다는 빠르지 않다. 하지만 동등한 조건으로만 비교되므로 매우 빠른 레코드 조회 방법의 하나다.**
...
동증 조건을 WHERE 절에 명시됐기 때문에 조건에 일치하는 레코드가 1건이라는 보장이 없다, 그래서 const가 아니라 ref 접근 방법이 사용됐으며 실행 계획의 ref 칼럼 값에는 const가 명시됐다. **이 const는 접근 방식이 아니라 ref ㅣㅂ교 방식으로 사용된 입력 값이 상수(d005)였음을 의미한다.**

* const: **조인의 순거에 관계없이 프라이머리 키나 유니크 키의 모든 칼럼에 대해 동등 조건으로 검색(반드시 1건의 레코드만 반환)**
...
그렇게 흔하게 발생하는 쿼리는 아니기 떄문에 해당 접근 방식은 쉽게 접하지는 않을 것이다.

#### unique_subquery

**WHERE 조건에서 사용될 수 있는 IN(subquery) 형태의 쿼리를 위한 접근 방식이다. unique_subquery의 의미 그대로 서브 쿼리에서 중복되지 않은 유니크한 값만 반환할 떄 이 접근 방법을 사용 한다.**
...
IN 연산자의 특성상, IN(subquery) 또는 IN(상수 나열) 형태의 조건은 괄호 안에 있는 값의 목록에서 중복된 값이 먼저 제거돼야 한다. 방금 살펴본 unique_subqery 접근 방법은 IN(subqeury) 조건의 subqery가 중복된 값을 만들어내지 않는다는 보자이 있으므로 별도의 중복을 제거할 필요가 없다. **하지만 IN(subqery)에서 subquery가 중복된 값을 반환할 수는 있지만 중복된 값울 인덱스를 이용해 제거할 수 있는 때 index_subquery 접근 방법이 사용된다.**
...
#### range
**ragne는 인덱스를 하나의 값이 아니라 범위로 검색하는 경우를 의미하는데 주로 <>, IS NULL, BETWEEN, IN, LIKE 등의 연산자를 이용해 인덱스를 검색할 때 사용된다. 일반적으로 애플리케이션의 쿠러ㅣ가 가장 많이 사용하는 접근 방법이다.** 이 책에서는 접근 속도의 순서상만 보면 상당히 느린것으로 보이지만 **이 접근 방법도 상당히 빠르며, 모든 쿼리가 이 접근 방법만 사용해도 어느정도 성능은 보장된다고 볼 수 있다.**
...
> 주의!
> **이 책에서 인덱스 레인지 스캔이라고 하면 const, ref, range라는 세 가지 접근 방법을 모두 묶어서 지칠하는 것이다. 또한 인덱스를 효율적으로 사용한다 또는 범위 재현 조건으로 인덱스를 사용한다 라는 표현 모두 이 세가지 접근 방법을 의미한다. 업무상 개발자가 DBA와 소통할 떄도 const, ref, ragne 접근 방법을 구분해서 언급하는 경우는 거의 없으며, 일반저그올 인덱스 레인지 스캔 또는 레인지 스캔으로 언급한다.**


#### index_merge
index_merge 접근 방식은 2개 이상의 인덱스를 ㅣㅇ용해 각각의 검색 결과를 만들어낸 후 그 결과를 병합하는 처리 방식이다. 하지만 여러 번의 경험을 보면 이름만큼 그렇게 효율저긍로 동작하지 것 같다. index_merge 접근 방식에 다음과 같은 특징이 있다.

* 여러 인덱스를 익어야 하므로 일반적으로 ragne 접근 방식보다 효율성이 떨어진다.
...
**접근 방식이 index라서, index를 효율적으로 사용할거 같은 오해가 있다. 하지만 index 접근 방식은 인덱스를 청므부터 끝까지 읽은 인덱스 풀 스캔을 의미한다. range 접근 방식고 ㅏ같이 효유렂ㄱ으로 인덱스의 필요한 부분만 읽는 것을 의미하는 것은 아니라는 점을 알아야 한다.**

index를 접근 방식은 테이블을 청므부터 끝까지 읽는 풀 테이블 스캔 방식과 비교했을 떄는 비교하는 레코드 건수는 같다. 하지만 인덱스는 일반적으로 데이터 파일 전체 보다는 크기가 작어서 풀 테이블 스캔보다는 효율적이므로 풀 테이블 스캔보다는 빠르게 처리 된다. 또한 쿼리의 내용에 따라 정렬된 인덱스의 장점을 이용할 수 있이므로 풀 테이블 스캔보다 훨씬 효울적으로 처리될 수 있다. index 접근 방법은 다음의 조건 가운데 (첫 번째 + 두 번째) 조건을 충족 하거나 (첫 번째 + 세 번째) 조건을 충족하는 쿼리에서 사용되는 읽기 방식이다.
...
**테이블을 처음부터 끝까지 전부 읽어서 불필요한 레코드를 제거(체크 조건이 존재할 떄)하고 반환한다. 풀 테이블 스캔은 지금까지 설명한 접근 방법으로 처리할 수 없을 때 가장 마지막으로 선택되는 가장 비효율적인 방법이다.**
