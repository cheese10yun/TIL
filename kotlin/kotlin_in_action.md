...
### Unit 타입: 코틀린의 void
코틀린의 Unit 타입은 자바 void와 같은 기능을 한다. **Unit은 모든 기능을 갖는 일반적인 타입이며, void와 달리 Unit을 타입 인자로 쓸 수 있다. Unit 타입에 속한 값은 단 하나뿐이며 그 이름도 Unit이다.** 다음과 같은 제네릭 파라미터를 반환하는 함수를 오버라이드하면서 반환 타입으로 Unit을 쓸 떄 유용하다.
...
경우에 따라서 널이 될 수 있는 값으로 이뤄진 넌ㄹ이 될 수 있는 리스트를 정의 할 떄는 물음표 2개를 사용한다. `List<Int?>?`
