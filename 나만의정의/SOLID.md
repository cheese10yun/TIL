...
## 역할
* 유저들은 그들이 수행하는 role에 따라 나눠야
* User가 특성 Role을 수행할때 Actor라고 부른다.
* 특정 액터의 요구사항을 만족시키기ㅣ 위한 인련의 함수의 집합
* Actor의 요구사항이 변경이 일련의 함수의 변경 근원이된다.



## SRP 
* 모듈은 하나/반드시 **하나의 변경 사유를 가져야한다**
    * 동일한 이유로 변경되어야 하는 것은 동일 모듈에
    * 다른 이유로 변경되어야 하는 것은 다른 모듈에
