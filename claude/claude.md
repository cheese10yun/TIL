...
```
/rename user-auth-feature          # 세션에 이름 부여 (나중에 찾기 쉽게)
/plan implement JWT login flow     # 코드 짜기 전에 설계 먼저 검토
/add-dir ../shared-libs            # 관련 패키지 디렉토리 추가
```

**시나리오**: 로그인 기능 새로 만들 때, 바로 코드 짜기 전에 `/plan`으로 설계 방향 합의 → 구현 완료 후 `/diff`로 변경사항 확인 → `/security-review`로 인증 관련 취약점 점검

---

### 2. 버그 수정 중 방향이 여러 갈래일 때

```
/branch approach-a                 # 현재 시점에서 브랜치 분기
# 방법 A 시도
/rewind                            # 마음에 안 들면 되감기
/branch approach-b                 # 다른 방향으로 재시도
```

**시나리오**: DB 쿼리 성능 개선 방법이 인덱스 추가 vs 쿼리 재작성으로 갈릴 때 `/branch`로 각각 시도해보고 비교
...
/compact focus on payment module   # 결제 모듈 관련 내용만 남기고 압축
/cost                              # 지금까지 쓴 토큰/비용 확인
```
ㅊ
**시나리오**: 대규모 리팩토링 작업이 길어져 컨텍스트가 80% 이상 찼을 때 `/compact`로 핵심 내용만 압축해 계속 진행
...
```
/resume                            # 세션 선택기에서 어제 세션 선택
/resume payment-refactor           # 세션 이름 알면 바로 지정
```

**시나리오**: 퇴근 전 작업하던 PR 브랜치로 다음날 다시 접속 → `/resume`으로 컨텍스트 복원 후 바로 이어서 작업
...
```
/security-review                   # 현재 변경사항 보안 취약점 자동 분석
/diff                              # 변경사항 대화형 뷰어로 검토
```

**시나리오**: 외부 API 연동 코드 작성 후 커밋 전에 `/security-review`로 API 키 노출, 입력값 검증 누락 등 점검

---

### 6. 빠른 사이드 질문 (컨텍스트 오염 없이)

```
/btw what's the default timeout in Spring RestTemplate?
/btw HTTP 상태코드 422 의미가 뭐더라?
```

**시나리오**: 구현 중 갑자기 궁금한 것이 생겼을 때 `/btw`를 쓰면 대화 기록에 남지 않아 현재 작업 흐름이 끊기지 않는다
...
```
/copy                              # 마지막 응답(코드, 설명 등) 클립보드 복사
/copy 2                            # 두 번째 최근 응답 복사
/export session-summary.txt        # 전체 대화 텍스트로 내보내기
```

**시나리오**: Claude가 생성한 마이그레이션 스크립트를 `/copy`로 복사해 팀 Slack에 붙여넣기, 또는 `/export`로 설계 논의 내용을 문서화
...
```
/effort max                        # 복잡한 아키텍처 설계 시 최대 사고력
/effort low                        # 간단한 오타 수정, 빠른 답변용
/fast on                           # 빠른 출력이 필요할 때
/model opus                        # 어려운 문제는 더 강력한 모델로
/model sonnet                      # 일반 작업은 기본 모델로 비용 절약
```

**시나리오**: 복잡한 분산 시스템 설계 논의는 `/effort max` + `/model opus`, 단순 코드 포맷팅은 `/effort low` + `/fast on`으로 속도 우선
...
```
/init                              # 프로젝트에 CLAUDE.md 생성 (컨벤션, 구조 문서화)
/memory                            # Claude가 기억해야 할 프로젝트 규칙 편집
/install-github-app                # GitHub Actions에 Claude 연동
```

**시나리오**: 신규 프로젝트 시작 시 `/init`으로 CLAUDE.md 생성해 코딩 컨벤션, 아키텍처 설명 기록 → 이후 모든 대화에서 자동으로 컨텍스트 로드
...
```
/doctor                            # Claude Code 자체 문제 진단
/status                            # 현재 모델, 계정, 연결 상태 한눈에 확인
/usage                             # 속도 제한 걸렸나 확인
/extra-usage                       # 한도 초과 시 추가 사용량 구매
```

**시나리오**: Claude가 갑자기 느려지거나 응답이 이상할 때 `/doctor` → `/status` 순서로 원인 파악
...
| 빈도 | 명령어 | 주요 상황 |
|------|--------|----------|
| 매일 | `/resume`, `/clear`, `/compact` | 세션 관리, 컨텍스트 관리 |
| 자주 | `/plan`, `/diff`, `/rewind` | 기능 개발, 코드 검토, 실수 복구 |
| 가끔 | `/security-review`, `/branch`, `/export` | 배포 전 점검, 실험, 문서화 |
| 필요시 | `/effort`, `/model`, `/fast` | 작업 복잡도에 따른 성능 조절 |
