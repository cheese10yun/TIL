...
2. **PQL은 SQL로 변환 되어 데이터베이스를 조회한다. (:exclamation:영속성 컨텍스트 부터 가는 것이 아니라 데이터베이스부터 간다)**
...
2. 더 분째 JPQL을 호출하면 데이터베이스에서 같은 회원 엔티티르르 조회한 한다. 이후 영속성 컨텍스트에는 이미 존재하는 엔티티이므로 존재하는 결과는 버린다.
