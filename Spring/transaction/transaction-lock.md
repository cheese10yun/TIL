...
위 그림은 Isolcation Level이 `REPEATABLE_READ`의 경우 **DB는 조회는 되었지만 아직 커밋하지 않은 로우에 읽기 잠금을 걸어둔 상태입니다.** 즉 스레드 1이  애그리거트를 구하는 동안 접근 잠금을 걸고 애그리거트를 수정하는 동안 스레드2에서는 잠금 대기 상태이기 때문에 잠금된 동안은 블록킹이 됩니다.

**하지만 JPA 환경에서는 `REPEATABLE_READ`의로도 해당 문제는 해결할 수 없습다.**

![](https://github.com/cheese10yun/TIL/raw/master/assets/transacion-isolration-jpa.png)

JPA여서는 트랜잭션 마다 영속성 컨텍스트를 갖게 됩니다. 위 그림처럼 각각의 영속성 컨텍스트 1차 캐시에 저장된 주문 객체를 영속성 컨텍스트A는 배송 상태를 변경, 영속성 컨텍스트B는 주소를 변경하려고 합니다. 결과적으로 모두 Commit이 되어 데이터베이스에 반영되게 된다.
